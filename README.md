...
- [Remote Administration Terminal (docs/ADMIN_TERMINAL.md)](docs/ADMIN_TERMINAL.md)
...
- Heltec WiFi LoRa 32 V4 + TFT expansion kit (`heltec-v4`): touch-first UI in either orientation — landscape or portrait is a setting on the device (Config → Orientation), not a separate build. GPS and full mesh UI support (channels, ANN, DMs, MAP, CFG, web config), and an optional external BLE keyboard; no microSD slot, so config, DM history and the node archive live in internal flash.
