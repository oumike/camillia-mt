...
Fixed
- Moving a device from a ten-channel build back to an eight-channel one (or restoring a Cardputer's saved config) no longer wipes every channel - the first eight keep their names and keys, and only the per-channel hop overrides fall back to the device default.)CAMNOTES";
