...
### Fixed
- Traceroutes aimed at a Camillia node now report the signal quality that node heard the request at. Previously the reply echoed the request unchanged, so a direct traceroute between two Camillia nodes had no signal readings at all.
