...
- Web config: importing a config while on the device's access point now returns a small confirmation page instead of reloading the full config page, which could fail on the limited memory available there.
